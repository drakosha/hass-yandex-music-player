...
- **Поиск** — поиск по всему каталогу Яндекс Музыки (исполнители, альбомы, треки, плейлисты)
- **Воспроизведение на любом плеере** — Kodi, DLNA, AirPlay, Chromecast, Sonos, VLC и др.
- **HTTP-прокси для DLNA** — старые DLNA-рендереры (NAD, Denon и др.), не поддерживающие HTTPS, получают аудио через встроенный прокси
- **Очередь воспроизведения** — плейлисты и альбомы играются целиком с автоматическим переходом между треками
- **Радио / Моя волна** — бесконечное воспроизведение с подгрузкой новых треков; «Моя волна» загружается автоматически при старте
...
2. Добавьте `https://github.com/drakosha/hass-yandex-music-player`, тип: **Integration**
3. Найдите «Yandex Music Player» и нажмите **Скачать**
...
```bash
cd /config
git clone https://github.com/drakosha/hass-yandex-music-player.git /tmp/ymp
cp -r /tmp/ymp/custom_components/yandex_music_player custom_components/
rm -rf /tmp/ymp
```
